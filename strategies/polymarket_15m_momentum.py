"""
Polymarket 15-minute BTC Up/Down Trading Bot
Momentum strategy: follows window delta + tick trend.
"""
import logging
import time
...
class Polymarket15MomentumStrategy(BaseStrategy):
    ...
    description = "15min BTC Momentum: window delta + tick trend"
    MIN_CONFIDENCE = 0.25
    WINDOW_SECONDS = 900
    SNIPE_OFFSET = 10
    ...
    def run(self):
        ...
        logger.info(f"[15m_momentum] window_ts={window_ts} secs_to_close={secs_to_close}")

        if secs_to_close <= 0:
            return

        if secs_to_close > self.SNIPE_OFFSET + 30:
            logger.info(f"[15m_momentum] Too early ({secs_to_close}s remaining)")
            return

        if not market:
            market = self._find_window_market(window_ts)
        if not market:
            logger.info(f"[15m_momentum] No market found")
            return
        ...
        if best_signal and best_signal.confidence >= self.MIN_CONFIDENCE:
            logger.info(f"[15m_momentum] BUY {best_signal.outcome} @ {best_signal.price:.3f} conf={best_signal.confidence:.2f} reason={best_signal.reason}")
            self.portfolio.buy(
